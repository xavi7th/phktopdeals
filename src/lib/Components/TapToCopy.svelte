...
<div
  class="tap-to-copy group relative inline-flex cursor-pointer items-center"
  ...
  <span class="copy-icon relative">
    {#if copied}
      ...
      <span class="success-tooltip absolute -top-8 left-1/2 -translate-x-1/2 transform rounded bg-green-700 px-2 py-1 text-xs text-white opacity-90">
        {successMessage}
      </span>
    {:else}
      <!-- Copy icon -->
      <span>
        <svg
          ...
          stroke-linecap="round"
          stroke-linejoin="round"
          class="ml-2 text-gray-500 group-hover:text-gray-700 {className}">
