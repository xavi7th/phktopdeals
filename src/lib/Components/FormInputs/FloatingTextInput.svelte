...
<script>
  ...
  let { class: className = undefined, name = "input-" + crypto.randomUUID().replaceAll("-", "").substring(0, 10), isError = true, label = "", placeholder = " ", gray = false, togglePw = undefined, strongPw = undefined, size = "p-4", value = $bindable(undefined), msg = [], ...rest } = $props();
</script>
