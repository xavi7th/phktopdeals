<script>
  ...
  let action = $state(''), nextInstructions = $state(false);
            ...
            {:else if paymentMethod === 'manual'}
              ...
              <section class="py-3">
                <div class="px-4 mx-auto max-w-7xl">
                  <ul class="max-w-md mx-auto mt-2 space-y-12">
                    {#if !nextInstructions}
                      ...
                      <li class="relative flex items-start">
                        <div class="relative flex items-center justify-center flex-shrink-0 w-16 h-16 bg-gray-100 dark:bg-neutral-700 rounded-full shadow">
                          <button class="bg-[#a18207] text-white px-8 py-6 rounded-lg font-medium hover:bg-[#8b7006] transition-all relative group flex items-center justify-center shadow-lg hover:shadow-xl active:shadow-md min-w-16 scale-50">
                            <svg width="40" height="40" viewBox="0 0 60 60" class="text-white absolute">
                              <!-- Animated arrow -->
                              ...
                              <!-- Circular path -->
                              <circle cx="30" cy="30" r="25" fill="none" stroke="currentColor" stroke-width="2" stroke-dasharray="4 4" class="opacity-50 group-hover:opacity-70 transition-opacity" />
                            </svg>
                          </button>
                        </div>
                        <div class="ml-6">
                          ...
                          <p class="mt-4 text-sm text-gray-600 dark:text-gray-400">
                            Click the "I have made payment" button below to proceed to the next step.
                          </p>
                        </div>
                      </li>
                    {/if}
      ...
      {#if paymentMethod === 'manual'}
        {#if !nextInstructions}
          <LoadingButton
            ...
            type="button"
            class="w-auto bg-success-icon px-3 py-2 font-medium transition-opacity duration-300 hover:bg-gray-700 hover:text-neutral-50 focus:bg-gray-700
            {$form.amount <= 0 ? 'pointer-events-none' : 'opacity-100'}">
            I have made Payment
          </LoadingButton>
        {:else}
