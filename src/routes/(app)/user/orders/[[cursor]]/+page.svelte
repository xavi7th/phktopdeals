...
<div class="grid grid-cols-3 gap-6 sm:mx-10 sm:grid-cols-5">
  {#each orderTabs as tab}
    <button
      class="grid place-content-center rounded-xl bg-white p-[16px] text-slate-800 shadow-lg dark:bg-neutral-800 dark:text-slate-100 border dark:border-neutral-600"
...
<Table
        ...
        <div class="py-3 pe-6 ps-6 lg:ps-3 xl:ps-0">
          <div class="flex items-center gap-x-3">
            ...
            <div class="flex grow flex-col gap-y-1">
              ...
              <span class="block text-wrap text-xs text-gray-800 dark:text-neutral-500 font-semibold">
                <span class="font-light text-gray-500">Transaction ID:</span>
                #{order.id}
              </span>
              {#if order.is_processed}
                <a href="/user/orders/details/{order.id}"
                  class="flex w-32 items-center rounded-lg underline pt-2 text-sm text-gray-800 dark:text-neutral-400 dark:hover:text-neutral-300 dark:focus:text-neutral-300"
            ...
            <div class="flex grow flex-col">
              ...
              {#if ! order.is_processed && ! order.expired_at}
                <span class="block text-wrap text-sm text-gray-500 dark:text-neutral-500">
                  <span class="font-sem-bold text-gray-800">Valid Until:</span> { new Date(order.valid_until || '').toLocaleDateString() }
                </span>
              {/if}
              {#if order.expired_at}
                <span class="block text-wrap text-sm text-red-600 dark:text-red-600">
                  <span class="font-sem-bold text-red-600">Expired:</span> { new Date(order.expired_at).toLocaleDateString() }
                </span>
              {/if}
            </div>
          </div>
  ...
  <svelte:fragment slot="mobile">
    {#each pageData.data as order}
      <div class="rounded-lg border border-gray-200 bg-slate-50 dark:bg-neutral-800 dark:border-neutral-700 dark:shadow-md dark:shadow-neutral-700/40 p-4 shadow-md">
        <div class="-m-4 flex items-center justify-between border-b border-gray-200 dark:border-neutral-700 p-4">
...
<Modal title="Details" name="view-order-details" on:close={() => {setTimeout((() => history.back()), 600)}}>
  <div slot="content">
