<script>
  ...
  import { unviewedCount, visibleNotifications, removeNotification, decrementUnviewed } from "$lib/stores/notificationStore.js";
...
<div class="min-h-screen bg-gray-50 dark:bg-neutral-900">
  ...
  {#each $visibleNotifications as notification (notification.id)}
    <div class="fixed end-3 top-20 z-50 space-y-2">
      <NotificationToast title={notification.title} message={notification.message} type={notification.type} conversationId={notification.conversationId} on:click={handleNotificationClick} />
    </div>
  {/each}
</div>
