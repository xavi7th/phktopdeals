...
<div class="col-span-5 lg:col-span-4 lg:col-start-2 px-4 py-10 sm:px-6 lg:px-8 lg:py-14 mx-auto lg:min-w-[70%]">

  <SuperDebug data={{message: form?.message, $message, $formData, $errors}} label="My form data" collapsible={true} display={dev} />
  ...
  <div class="bg-white rounded-xl shadow p-4 sm:p-7 dark:bg-neutral-900">
    <form method="POST" enctype="multipart/form-data" use:enhance>
      <div class="grid grid-cols-12 gap-y-8 py-8 first:pt-0 last:pb-0 border-t first:border-transparent border-gray-200 dark:border-neutral-700 dark:first:border-transparent">
        ...
        <div class="col-span-12">
          <FloatingSelectInput class="flex-1" name="product_type" label="Product Type" bind:value={$formData.product_type} options={['Gift Card','Game','eSim','Top Up']} isError={ !! $errors.product_type} msg={$errors.product_type}/>
        </div>
        ...
        <div class="col-span-12 flex gap-x-2">
          <FloatingSelectTagAltInput class="flex-1" name="product_category" label="product category" bind:value={$formData.product_category} isError={ !! $errors.product_category} msg={$errors.product_category?._errors} multiple>
            {#each ['entertainment', 'fashion', 'groceries', 'food', 'beauty', 'electronics', 'payment-cards', 'subscriptions', 'software', 'sports', 'security', 'travel', 'games', 'top-up', 'eSims', 'hot sales', 'streaming'] as item}
