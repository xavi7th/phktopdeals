    ...
    <div class="grid grid-cols-12 gap-y-8 border-t border-gray-200 py-8 first:border-transparent first:pt-0 last:pb-0 dark:border-neutral-700 dark:first:border-transparent">
      ...
      <div class="col-span-12 flex">
        {#if $form.product_image_url}
          <a href={$form.product_image_url} target="_blank" rel="noopener noreferrer">
            <img src={$form.product_image_url} alt="" class="h-16 w-16 rounded-xl" />
          </a>
        {/if}

        <FloatingFileInput
          class="grow {$form.product_image_url ? 'ml-5' : ''}"
          name="product_image"
          label="Product Image (300 * 300)"
          accept="image/*"
          bind:files={$form.product_image}
          isError={!!$errors.product_image}
          msg={$errors.product_image} />
      </div>
