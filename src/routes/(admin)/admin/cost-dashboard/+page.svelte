...
<div class="mx-auto max-w-6xl space-y-6 p-4">
    <div class="flex items-center justify-between">
        ...
        <div class="flex items-center gap-3">
            <input
                type="date"
                bind:value={fromDate}
                class="rounded-lg border border-gray-200 px-3 py-2 text-sm dark:border-neutral-700 dark:bg-neutral-800 dark:text-neutral-200"
            />
    ...
    {#await costData}
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {#each Array(4) as _}
                <div
                    class="h-24 animate-pulse rounded-xl border bg-gray-100 dark:border-neutral-700 dark:bg-neutral-800"
                ></div>
            {/each}
        </div>
    {:then costs}
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <div
                class="rounded-xl border bg-white p-4 shadow-sm dark:border-neutral-700 dark:bg-neutral-800"
            >
            ...
            >
                ...
                <p class="mt-1 text-2xl font-bold text-gray-900 dark:text-neutral-100">
                    {formatUsd(
                        costs.totals?.unique_conversations > 0
                            ? costs.totals.total_cost / costs.totals.unique_conversations
                            : 0
                    )}
                </p>
            </div>
            ...
            >
                ...
                <p class="mt-1 text-2xl font-bold text-gray-900 dark:text-neutral-100">
                    {formatNumber(
                        (costs.totals?.total_prompt_tokens || 0) +
                            (costs.totals?.total_completion_tokens || 0)
                    )}
                </p>
            </div>
            <div
    ...
    {:catch error}
        <div
            class="rounded-xl border border-red-200 bg-red-50 p-4 text-red-600 dark:border-red-800 dark:bg-red-950 dark:text-red-400"
        >
