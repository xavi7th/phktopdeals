...
<div class="flex justify-between border-b border-gray-200 dark:border-neutral-700 px-6 py-4">
...
{:then sliders}
  <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
    {#each sliders.data || [] as slider}
      <div class="flex justify-between flex-col rounded-lg border border-gray-200 bg-slate-50 p-4 shadow-md dark:border-neutral-700 dark:bg-neutral-800 dark:shadow-md dark:shadow-neutral-700/40">
        ...
        <div class="-mx-3 flex items-center justify-between border-t border-gray-200 p-4 pb-0 dark:border-neutral-700">
          <button
            class="inline-flex items-center gap-x-1 font-medium text-neutral-700 focus:outline-none dark:text-white rounded-md px-3 py-1 text-xs w-auto bg-orange-600 hover:bg-orange-700 focus:bg-orange-700"
            aria-haspopup="dialog"
            aria-expanded="false"
            aria-controls="slider-modal"
            data-hs-overlay="#slider-modal"
            on:click={() => {(curSlider = slider)}}>
            EDIT
          </button>
          ...
          <ConfirmAction action="?/delete" formData={data.form} msg="Are you sure you want to delete this slider?">
            ...
            <LoadingButton class="bg-red-600 py-1 px-2 text-xs uppercase hover:bg-gray-100 text-white hover:text-red-600 hover:border-red-600 focus:bg-gray-100 focus:text-red-600 focus:border-red-600 rounded-md" label="DELETE"/>
          </ConfirmAction>
        </div>
      </div>
    {:else}
