<script>
  ...
  let btnColor = $derived(
    slider.id ? "bg-amber-700 hover:bg-amber-800 hover:text-neutral-50 focus:bg-amber-800 border-amber-600" : "bg-teal-900 hover:bg-teal-800 hover:text-neutral-50 focus:bg-teal-800 border-teal-600/50",
  );
</script>
...
<Modal title="{slider.id ? 'Edit Existing' : 'Create'} Slider" name="slider-modal">
    ...
    <form id="slider-form" method="POST" action={slider.id ? "?/update" : "?/create"} enctype={hasFile ? "multipart/form-data" : "text/plain"} use:enhance>
      <div class="flex flex-col gap-y-3 p-4">
        ...
        <div class="flex flex-1 items-center gap-x-2">
          <img class="inline-block h-16 w-16 rounded-full" src={slider.img_url} alt="Slider img" />
  ...
  <svelte:fragment slot="footer">
    <LoadingButton form="slider-form" class="w-auto px-3 py-2 font-medium {btnColor} transition-colors duration-300" label="{slider.id ? 'Edit' : 'Create'} Slider" {timeout} {delayed} {submitting} />
  </svelte:fragment>
</Modal>
