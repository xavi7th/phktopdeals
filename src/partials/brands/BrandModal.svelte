<script>
  ...
  export let title = "Create New Brand",
    delay = 3000,
    form;
  /**
   * @type {any[]}
            ...
            <table class="min-w-full divide-y divide-gray-200 dark:divide-neutral-700">
              ...
              <tbody class="divide-y divide-gray-200 dark:divide-neutral-700">
                {#each brands || [] as brand (brand.id)}
                  <tr>
                    <td class="whitespace-nowrap px-6 py-4 text-sm font-medium text-gray-800 dark:text-neutral-200">{brand.name}</td>
...
<Modal {title} name="hs-static-edit-modal">
  ...
  <div slot="footer">
    <button
      type="button"
      id="backButton"
      class="inline-flex items-center gap-x-2 rounded-lg border border-transparent bg-slate-600 px-3 py-2 text-sm font-medium text-white hover:bg-slate-700 focus:bg-slate-700 focus:outline-none disabled:pointer-events-none disabled:opacity-50"
      aria-haspopup="dialog"
      aria-expanded="false"
