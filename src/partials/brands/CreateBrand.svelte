...
<script>
   ...
   */
    ...
    * @type {Props}
    */
  let { create = false, brandForm, brand = brandDefaults, name = crypto.randomUUID().replaceAll("-", "").substring(0, 10), onclick = e => {(brand = brandDefaults)} } = $props();

  let mounted = $state(false);
  let actionBtnColor = $derived(brand.id ? 'bg-amber-700 hover:bg-amber-800 hover:text-neutral-50 focus:bg-amber-800 border-amber-600 py-1 px-2' : 'bg-teal-900 hover:bg-teal-800 hover:text-neutral-50 focus:bg-teal-800 border-teal-600/50');
  let triggerBtnColor = $derived(create ? 'w-40 font-medium text-white px-4 py-3 bg-brand-600 hover:bg-brand-700 focus:bg-brand-700' : 'font-semibold text-brand-600 hover:text-brand-800 focus:text-brand-800 dark:text-brand-500 dark:hover:text-brand-400 dark:focus:text-brand-400');
...
<button
  ...
  data-hs-overlay="#brand-modal" {onclick}>
  {#if create} {@html plusIcon} {/if}
  ...
  <Portal shouldMount={mounted}>
    <Modal title="{brand.id ? 'Edit Existing' : 'Create'} Brand" name="brand-modal" on:close={() => (brand = brandDefaults)}>
      ...
      <svelte:fragment slot="footer">
        <LoadingButton form="brand-form" class="w-auto py-2 px-3 font-medium {actionBtnColor} transition-colors duration-300" label="{brand.id ? 'Edit' : 'Create'} Brand"  {timeout} {delayed} {submitting}/>
      </svelte:fragment>
    </Modal>
  </Portal>
{/if}
