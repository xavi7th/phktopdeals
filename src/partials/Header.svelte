...
<header class="fixed bg-transparent z-30 w-full">
  <!-- lg+ -->
  <div class="bg-white dark:bg-neutral-950/30 border-b border-gray-200 dark:border-neutral-800 drop-shadow-md dark:shadow-gray-600">
    <div class="px-4 mx-auto sm:px-6 lg:px-8">
      <nav class="relative flex items-center justify-between h-16 lg:h-20">
        ...
        <div class="lg:absolute lg:-translate-x-1/2 lg:inset-y-5 lg:left-1/2 scale-75 -ml-5">
        ...
        <div class="inline-flex lg:hidden">
          ...
          <button type="button" class="inline-flex p-2 ml-5 text-black dark:text-gray-200 transition-all duration-200 rounded-md lg:hidden focus:bg-gray-100/10 hover:bg-gray-100/10" onclick={() => showMenu = !showMenu}>
        ...
        <div class="hidden lg:flex lg:items-center lg:space-x-6">
          {#if user?.full_name}
            <a href="/user/orders" class="text-base text-gray-700 hover:text-brand-600 dark:text-neutral-300 font-medium tracking-tighter"> Dashboard </a>
            <a href="/logout" class="text-base text-gray-700 hover:text-brand-600 dark:text-neutral-300 font-medium tracking-tighter" data-sveltekit-reload> Logout </a>
          {:else}
            <a href="/login#register" class="text-base text-gray-700 hover:text-brand-600 dark:text-neutral-300 font-medium tracking-tighter"> Sign Up </a>
  ...
  <!-- xs to lg -->
   {#if showMenu}
    <nav class="py-4 bg-white lg:hidden" transition:slide={{ axis: 'y' }}>
      <div class="px-4 mx-auto sm:px-6 lg:px-8">
        <div class="flex items-center justify-between">
          ...
          <button type="button" class="inline-flex p-2 text-black transition-all duration-200 rounded-md focus:bg-gray-100 hover:bg-gray-100" onclick={() => showMenu = false}>
        ...
        <div class="mt-6">
          ...
          <div class="flex flex-col space-y-2">
            {#if user?.full_name}
              <a href="/user/orders" class="py-2 text-base font-medium text-black transition-all duration-200 focus:text-brand-600 hover:text-brand-600"> Dashboard </a>
              <a href="/logout" class="py-2 text-base font-medium text-black transition-all duration-200 focus:text-brand-600 hover:text-brand-600" data-sveltekit-reload> Logout </a>
            {:else}
              <a href="/login#register" class="py-2 text-base font-medium text-black transition-all duration-200 focus:text-brand-600 hover:text-brand-600"> Sign up </a>
              <a href="/login" class="py-2 text-base font-medium text-black transition-all duration-200 focus:text-brand-600 hover:text-brand-600"> Sign in </a>
            {/if}
